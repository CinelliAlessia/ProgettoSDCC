...
func printDebugBlue(blueString string, message Message, clockServer main.ClockServer) {
	if common.GetDebug() {
...
func printGreen(greenString string, message Message, clockServer main.ClockServer) {
	// Ottieni l'orario corrente
	now := time.Now()
	// Formatta l'orario corrente come stringa nel formato desiderato
	formattedTime := now.Format(layoutTime)
...
func printRed(redString string, message Message, clockServer main.ClockServer) {
